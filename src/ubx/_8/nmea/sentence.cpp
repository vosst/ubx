...
nmea::Sentence nmea::parse_sentence(const std::string& message) {
    ...
    if (not boost::spirit::qi::parse(
                ...
                (
                    lit('$') >>
                    as_string[*(~char_('*'))][boost::phoenix::ref(s) = _1] >>
                    lit('*') >>
                    hex[ref(cs) = _1] >>
                    "\r\n"
                )))
...
std::string nmea::generate_sentence(const Sentence &sentence) {
  ...
  using boost::spirit::karma::upper;
    ...
    std::string s; std::back_insert_iterator<std::string> its(s);
    boost::spirit::karma::generate(its, Generator<std::back_insert_iterator<std::string>>(), sentence);

    std::string result; std::back_insert_iterator<std::string> itr(result);
    if (not boost::spirit::karma::generate(
                itr,
                lit('$') << s << lit('*') << upper[hex] << "\r\n",
